...
# Common stopwords (expanded for better coverage)
STOPWORDS = {
    "the", "and", "of", "to", "in", "a", "is", "for", "on", "with", "by", "at", "this", "that", "are", "be", "was", "were", "an"
}
...
class FileAnalyzer:
    ...
    def __init__(self, file_path: Path, show_progress: bool = False, encoding: str = 'utf-8', top_words: int = 10, exclude_stopwords: bool = False):
    ...
    def _process_line(self, line: bytes) -> None:
        ...
        try:
            ...
            with self.lock:
                ...
                words = re.findall(r"\b[a-zA-Z]+(?:['-][a-zA-Z]+)*\b", decoded_line.lower())
...
def print_results(analyzer: 'FileAnalyzer', show_word_freq: bool = False) -> None:
    """Print analysis results for a single file (kept for potential reuse)."""
...
def is_valid_file(file_path: Path) -> bool:
    ...
    if not (path.is_file() and os.access(path, os.R_OK) and mime_type and mime_type.startswith('text')):
        return False
    try:
...
def main() -> None:
    ...
    # Build table with optional word frequencies
    table_headers = ["File", "Lines", "Words", "Chars", "Bytes", "Longest Line"]
    table = [
    ...
    if args.words:
        ...
        for row, analyzer in zip(table, [a for a in analyzers if not a.error]):
            if analyzer.results["word_freq"]:
                top_words = ", ".join(f"{word}: {count}" for word, count in analyzer.results["word_freq"])
            ...
            with output_path.open('w', encoding='utf-8', newline='') as f:
                if args.output == 'json':
                    ...
                    for a in analyzers:
                        ...
                        if args.words:
                            top_words = ", ".join(f"{word}: {count}" for word, count in a.results["word_freq"]) if a.results["word_freq"] else "N/A"
                            row.append(top_words)
                        writer.writerow(row)
